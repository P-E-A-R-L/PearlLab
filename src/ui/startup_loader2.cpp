...
void StartupLoader::load_graph()
{
    ...
    auto limeNode = new PipelineGraph::Nodes::PythonModuleNode(modules["LimeExplainability"]);
    ...
    PipelineGraph::addLink(limeNode->inputs[0].id, device->outputs[0].id);
    ...
    strcpy(limeAcceptor->_tag, methodTag2.c_str());
